...
The router cannot read its own runtime effort, so it cannot distinguish "raise model" from "raise effort" at decision time. It surfaces only the direction ("this may need higher power") and leaves the dimension choice to the user. This is what empties ground (A) above; the blind spots that force it, and the rest of the gate's shape, are [ADR-0011](0011-gate-direction-only-read-from-externals.md)'s.
