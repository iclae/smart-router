...
1. **Spawn on a power *or* context-economy driver, gated by separability and a floor** — spawn when *either* the needed model differs from the baseline enough to matter (down to Sonnet/Haiku) *or* context economy calls for it (ADR-0007), *and* the task is a self-contained sub-problem whose result returns as a compact artifact, *and* it clears the floor (small enough that main just does it → the fixed brief+cold-start cost outweighs the saving, so don't spawn). A hard task that is the main through-line is not spawned (it would make the weaker main a relay for a result it can't evaluate). Pure effort is never on its own a spawn trigger: a task that needs only *more* effort on the through-line is a human gate (the main cannot self-raise effort), and one that needs *less* just stays inline. (Baseline is variable per ADR-0010, not the fixed `Opus+high` this rule was originally written against.)
