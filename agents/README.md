...
```bash
cp agents/effort-*.md ~/.claude/agents/
```
...
Each declares `model: inherit`, so the model comes from the `model` argument at the spawn call and these stay pure effort containers — a vendor adding a model costs a Roster edit, not a file here (ADR-0001, ADR-0009).

The three tiers are **semantic** (how much thinking the task needs), not a snapshot of which model/effort pairs are cost-efficient today. That snapshot lives in the Roster's note and goes stale; these do not.
